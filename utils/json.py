            ...
            while self._peek() != '"':
                ...
                if ch == "\\":
                    ...
                    if ch in 'brnft':
                        ...
                        raise ReadException("Not a valid escaped JSON character: '%s' in %s" % (
    ...
    def _hexDigitToInt(self, ch):
        try:
            ...
            try:
                ...
                raise ReadException(
                    "The character %s is not a hex digit." % ch)
        return result
    ...
    def _readObject(self):
        ...
        while not done:
            ...
            if not isinstance(key, types.StringType):
                raise ReadException(
                    "Not a valid JSON object key (should be a string): %s" % key)
            self._eatWhitespace()
            ch = self._next()
            if ch != ":":
...
try:
    ...
    def yajl_write(obj, escaped_forward_slash=True):
        s = yajl.dumps(obj)
...
def format_output(func):
    from luzong.template import request

    def _(*a, **kw):
        r = func(*a, **kw)
        if request.get_form_var('alt') == 'xd':
            callback = request.get_form_var('callback', '')
            if callback:
                return "%s(%s)" % (callback, encode(r))
            else:
                return encode(dict())
                # raise RequestError('missing callback')
        else:
            return encode(r)
